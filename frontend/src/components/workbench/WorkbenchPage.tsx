      ...
      <div className="mx-auto max-w-[1500px] space-y-4 p-4">
        <section className="border border-slate-700 bg-slate-900 p-3" aria-label="Historical state controls">
          <div className="grid items-end gap-3 lg:grid-cols-[100px_minmax(220px,2fr)_110px_minmax(180px,1.4fr)_110px_auto]">
            ...
            <button onClick={() => void loadState()} disabled={loading !== null || decisionLap === null} className="h-[38px] whitespace-nowrap bg-cyan-600 px-4 text-xs font-bold uppercase tracking-wider text-slate-950 hover:bg-cyan-400 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400">{loading === "state" ? "Reconstructing…" : "Load state"}</button>
          </div>
          {loading && loading !== "state" && <p role="status" className="mt-2 text-xs text-slate-400">Loading {loading}… OpenF1 cold starts can take several seconds.</p>}
