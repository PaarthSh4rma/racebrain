...
## Decision
A historical application-level DecisionContext uses a full-field RaceState containing only observations demonstrably knowable by the cutoff. A focal completed lap may establish timestamp T; other competitors use their own latest defensible observation at or before T, not the focal lap number. Unknown timestamps are excluded when availability cannot be proven. Outcome loading occurs only after prediction is frozen.
